...
type ExcalidrawOnChange = NonNullable<
  ComponentProps<ExcalidrawComponent>['onChange']
>
...
function isRevisionConflict(
  error: unknown,
): error is RevisionConflictError {
  return (
...
function getBindableTarget(
  binding: unknown,
): BindableTarget | null {
  const candidate = Array.isArray(binding)
    ? binding[2]
    : binding
...
export function WhiteboardCanvas({
  ...
  const [
    excalidrawRuntime,
    setExcalidrawRuntime,
  ] = useState<ExcalidrawRuntime | null>(null)

  const [
    excalidrawLoadError,
    setExcalidrawLoadError,
  ] = useState<string | null>(null)

  const [boardState, setBoardState] =
    useState<BoardState>({ status: 'loading' })
  ...
  const [lockPoints, setLockPoints] =
    useState<LockPoint[]>([])

  const rootRef = useRef<HTMLDivElement>(null)

  const initialDataRef =
    useRef<ExcalidrawInitialDataState>(EMPTY_SCENE)
  ...
  const captureScene =
    useCallback((): ExcalidrawInitialDataState => {
      const api = apiRef.current
      const serializeAsJSON =
        excalidrawRuntime?.serializeAsJSON
  ...
  useEffect(() => {
    ...
    setExcalidrawLoadError(null)

    void import('@excalidraw/excalidraw')
      .then((runtime) => {
        if (!active) return

        setExcalidrawRuntime(() => runtime)
      })
      .catch((error: unknown) => {
        if (!active) return

        setExcalidrawLoadError(readableError(error))
      })
  ...
  const loadBoard = useCallback(async () => {
    ...
    try {
      ...
      const initialData = toInitialData(
        board.document.snapshot,
      )
  ...
  saveNowRef.current = async () => {
    ...
    try {
      const saved = await storage.save({
        id: whiteboardId,
        document:
          createDocumentFromSnapshot(snapshot),
  ...
  function handleSceneChange(
    ...args: Parameters<ExcalidrawOnChange>
  ) {
    if (!excalidrawRuntime) return
    ...
    const target = getBindableTarget(
      appState.suggestedBindings?.[0],
    )

    const editorRect = rootRef.current
      ?.querySelector('.rc-canvas__editor')
    ...
    if (
      sceneJson === lastSceneJsonRef.current
    ) {
      return
    }
  ...
  async function commitTitle() {
    ...
    if (
      !nextTitle ||
      nextTitle === boardState.board.title
    ) {
    ...
    try {
      const summary = await storage.rename(
        whiteboardId,
        nextTitle,
      )
  ...
  if (excalidrawLoadError) {
    return (
      <div
        className="rc-canvas-state rc-canvas-state--error"
        role="alert"
      >
        <Warning size={30} />

        <h2>
          The canvas editor could not be loaded
        </h2>

        <p>{excalidrawLoadError}</p>
      </div>
    )
  }
  ...
  if (boardState.status === 'error') {
    return (
      ...
      >
        ...
        <h2>
          This whiteboard could not be opened
        </h2>
    ...
    >
        ...
        <header className="rc-canvas__header">
          <div className="rc-canvas__identity">
            {onBack && (
              <button
                type="button"
                aria-label="Back to whiteboard library"
          ...
          <div className="rc-canvas__header-meta">
            <span className="rc-canvas__input-note">
              Pencil draws · touch moves
            </span>

            <span
              className="rc-save-status"
      ...
      <div className="rc-canvas__editor">
        ...
        {chrome !== 'none' &&
          activeTool === 'arrow' && (
            <div
              className="rc-connector-hint"
              role="status"
            >
              {lockPoints.length
                ? 'Release to lock'
                : 'Targets highlight when the connector locks'}
            </div>
          )}
        ...
        {chrome !== 'none' &&
          lockPoints.length > 0 && (
            <div
              className="rc-lock-points"
              aria-hidden="true"
            >
              {lockPoints.map(
                (point, index) => (
                  <i
                    className="rc-lock-point"
                    key={index}
                    style={{
                      left: point.x,
                      top: point.y,
            ...
            <div>
              ...
              <span>
                {saveStatus === 'conflict'
                  ...
                  : 'The local canvas is still intact.'}
              </span>
            </div>
